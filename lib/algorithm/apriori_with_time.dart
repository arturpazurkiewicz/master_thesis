import 'package:biedronka_extractor/algorithm/vector_prediction.dart';

import 'apriori.dart';

class AprioriWithTime extends Apriori {
  AprioriWithTime(minSupport, transactions) : super(minSupport, transactions);
  ...
  List<List<int>> prepareElements() {
    ...
    for (var transaction in transactions) {
      ...
      line.add(generateMonthProduct(transaction.recipe.time.month));
  ...
  Set<int> predictProductWithTime(Set<int> inputNames, DateTime day) {
    ...
    data.add(generateMonthProduct(cleanDay.month));
...
int generateMonthProduct(int month) {
  return -month;
}
